...
rotate() {
    ...
    # а под set -euo pipefail — весь скрипт (найдено на приёмке).
    # Пустой список — норма, поэтому листинг гасим (|| true), но САМО
    # удаление не маскируем: упавший rm обязан завалить бэкап громко.
    local old
